...
function deployDApp () {
  ...
  const deploy  = spawn(`${Utils.sudo()} npm run deploy:frontend:ipfs`, options)
...
function deployContract (network, mnemonic) {
  ...
  const migrate = spawn(`${Utils.sudo()} npm run deploy:contracts:${network}`, options)
...
module.exports = async function (cmd) {
  if (fs.existsSync(path.join(process.cwd(), './truffle.js'))) {
    ...
    if (cmd.contract) {
      const answer = (cmd.ropsten) ? 'Yes' : (await prompt({
        type: 'list',
        name: 'net',
